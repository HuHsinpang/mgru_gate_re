    ...
    with open(path_dataset) as f:
        ...
        for line in f:
            ...
            if line:
                ...
                sent = ' '.join(new_sentence).rstrip("'").replace(
                    " '", " ").replace("' ", " ")     # 去除引号
                sent = sent.replace("(", " ( ").replace(")", " ) ").replace(
                    "  ", " ")    # 将括号与连接单词分割开
                sent_remove_brackets = re.sub(r'\(.*?\)\ *', '', sent)
                if e1 in sent_remove_brackets and e2 in sent_remove_brackets:
                    sent = sent_remove_brackets
                relation = piece[1]
                dataset.append(((e1, e2, sent), relation))
                piece = list()
    return dataset
