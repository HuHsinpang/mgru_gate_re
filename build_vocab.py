...
parser.add_argument('--min_count_word', default=1,
                    help="Minimum count for words in the dataset", type=int)
parser.add_argument('--min_count_tag', default=1,
                    help="Minimum count for labels in the dataset", type=int)
...
if __name__ == '__main__':
    ...
    words = Counter()
    size_train_sentences = update_vocab(os.path.join(
        args.data_dir, 'train/sentences.txt'), words)
    size_test_sentences = update_vocab(os.path.join(
        args.data_dir, 'test/sentences.txt'), words)
    print("- done.")

    # Build label vocab with train and test datasets
    print("Building label vocabulary...")
    labels = Counter()
    size_train_tags = update_labels(os.path.join(
        args.data_dir, 'train/labels.txt'), labels)
    size_test_tags = update_labels(os.path.join(
        args.data_dir, 'test/labels.txt'), labels)
    print("- done.")
    ...
    words = sorted([tok for tok, count in words.items()
                    if count >= args.min_count_word])
    labels = sorted([tok+' '+str(count) for tok, count in labels.items() if count >= args.min_count_tag],
    ...
    save_dict_to_json(sizes, os.path.join(
        args.data_dir, 'dataset_params.json'))
